...
var _autoRoot=function(r){
 if('auto'!=I.ROOT) return;
 var n=r.indexOf('://');
 if(n>0) r=r.substr(n+3);
 n=r.indexOf('/');
 if(n<0) I.ROOT='';
 else{
  r=r.substr(n);
...
var lazyLoad=function(c,callback){
 ...
 o.onload=function(){
  try{this.parentNode.removeChild(this);}catch(e){return;}
  callback.call(callback,c);
 };
...
var checkLoad=function(callback){
 ...
 for(i in P){
  if(!P[i].loaded){
   W.setTimeout(function(){checkLoad(callback);},1);
   return;
  }
 }
 var cq={};
...
var preLoad=function(cs,f){
 if(SYN) W.setTimeout(function(){preLoad(cs,f);},1);
 else load(cs,f);
};
_dir();
var F=[
...
var _regist=function(c,cd){
 ...
 if(0==c.indexOf('lang.')) initClass(c,cd);
 else try{I.lang.Store.set('I.'+c,'I.regist(\''+c+'\','+cd+'+\'\');');}catch(e){}
};
var _depend=function(code){
 var c={},s=code,idx=s.indexOf('I.');
 ...
 for(i in c){
  j=i.indexOf('+');
  if(j>-1) i=i.substr(0,j);
  if(0==i.indexOf('lang.')||'ROOT'==i||'debug'==i||'version'==i||0==i.indexOf('isMobile')) continue;
  z.push(i);
 }
 return z;
};
var _want=function(callback){
 ...
 cs.length<1?callback.apply(callback):preLoad(cs,callback);
};
var _get=function(klass,callback){
 preLoad([klass],function(){
  var k=klass.split('.'),name=k.pop(),obj=I;
  for(var i=0,j=k.length;i<j;i++) obj=obj[k];
...
I.regist('lang.Store',function(w,d){
 ...
 var _LS=[{
  ...
  setItem:function(k,v){this.instance.setItem(k,v);},
  getItem:function(k){return this.instance.getItem(k);},
  clear:function(){this.instance.clear();},
  removeItem:function(k){
   var inst=this.instance,c=[],l=inst.length,i,key;
   switch(k.indexOf('*')){
    case 0:
     var suffix=k.substr(1);
     for(i=0;i<l;i++){
      key=inst.key(i);
      if(key.indexOf(suffix)==key.length-suffix.length) c.push(key);
     }
     break;
    case k.length-1:
     ...
     for(i=0;i<l;i++){
      key=inst.key(i);
      if(0==key.indexOf(prefix)) c.push(key);
     }
     break;
 ...
 var _get=function(k){
  for(var i=0,j=_LS.length;i<j;i++){
   if(_LS[i].support()) return _LS[i].getItem(k);
  }
  return null;
 };
 var _clear=function(){
...
I.regist('lang.Core',function(W,D){
 ...
 var E=[
  function(o,s,f){
   o.attachEvent('on'+s,function(e){EV(f,o,W.event||e);});
  },
  function(o,s,f){
   o.addEventListener(s,function(e){EV(f,o,W.event||e);},false);
  }
 ];
 var STYLE=[
  function(s){
 ...
 var G={
  ...
  NEXT:function(d,o){
   var pt=o.parentNode,chd=this.CHILD(pt),me=false;
  ...
  PREVIOUS:function(d,o){
   var pt=o.parentNode,chd=this.CHILD(pt),last=null;
   for(var i=0,j=chd.length;i<j;i++){
    if(chd[i]==o) return last;
    last=chd[i];
   }
   return null;
  },
  CLASS:function(o,s){
   if(o.getElementsByClassName) return o.getElementsByClassName(s);
   var c=[],els=this['TAG'](o,'*');
   var i=els.length;
   var ss=s.replace(/\-/g,'\\-');
 ...
 var _getStyle=function(o,s){return o.currentStyle?o.currentStyle[s]:W.getComputedStyle(o,null)[s];};
 var _each=function(l,f){
  var tp=Object.prototype.toString.apply(l);
  if(EACH[tp]||(l.length&&(!l.alert)&&('[object String]'!=tp))){
   for(var i=0,j=l.length;i<j;i++) f(l[i],i);
  }else f(l,0);
 };
 var _delay=function(time,callback){W.setTimeout(function(){callback.call(callback);},time);};
